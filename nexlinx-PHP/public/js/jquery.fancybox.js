	...
	$.extend(F, {
		...
		_afterLoad: function () {
			...
			if (F.isOpened) {
				...
				F.wrap.stop(true).removeClass('fancybox-opened');
				F.inner.css('overflow-y', 'visible');
		...
		_afterZoomIn: function () {
			...
			F.wrap.addClass('fancybox-opened').css('overflow-y', 'visible');

			F.update();

			F.inner.css('overflow-y', current.scrolling === 'auto' ? 'auto' : (current.scrolling === 'yes' ? 'scroll' : 'hidden'));
	...
	F.transitions = {
		getOrigPosition: function () {
			...
			if (element && element.nodeName && $(element).is(':visible')) {
				...
				viewport = F.getViewport();
				pos.top = viewport.y + (viewport.h - height) * 0;
				pos.left = viewport.x + (viewport.w - width) * 0;
			}
